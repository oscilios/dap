...
DAP_TEST_F(TaylorTest, sine_func)
{
    ...
    DAP_ASSERT_FLOAT_EQ(sequence_size::value, arr.size());
    ...
    for (auto value : arr)
    {
        ...
        DAP_ASSERT_FLOAT_EQ(std::sin(k * i), value);
        i++;
    }
}

DAP_TEST_F(TaylorTest, cosine_func)
{
    using precision = std::integral_constant<size_t, 31>;
    double x        = M_PI / 3.0f;
    double result = taylor::detail::cosine(x, precision::value);
    DAP_ASSERT_FLOAT_EQ(std::cos(x), result);
    ...
    {
        // std::cout << value << std::endl;
        DAP_ASSERT_FLOAT_EQ(std::cos(k * i), value);
        i++;
    }
}

DAP_TEST_F(TaylorTest, exp_func)
{
    using precision = std::integral_constant<size_t, 63>;
    double x        = M_PI / 3.0f;
    double result = taylor::detail::exp(x, precision::value);
    DAP_ASSERT_FLOAT_EQ(std::exp(x), result);
    ...
    for (auto value : arr)
    {
        ...
        DAP_ASSERT_FLOAT_EQ(std::exp(i), value);
        i += k;
    }
}
